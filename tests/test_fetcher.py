...
from scores3830 import BASE, IDENTIFY_HEADER, USER_AGENT, Fetcher
...
class TestHeaders:
    ...
    def test_sets_identifying_header_with_contact_info(self, tmp_path):
        f = Fetcher(cache_dir=tmp_path)
        for key, value in IDENTIFY_HEADER.items():
            assert f.session.headers[key] == value
        assert "@" in IDENTIFY_HEADER["X-Requested-By"]  # a real contact address, not hidden
