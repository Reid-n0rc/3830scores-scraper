...
LIMITATIONS (read before relying on this for anything):
  ...
  - Author email links are obfuscated on the page with HTML numeric-entity
    encoding specifically to defeat scraping; this tool decodes it to
    populate its own data model, not to build a mailing list.
  - Rate limited to ~1 req/sec by default and cached on disk - a courtesy
    default so this reads politely rather than hammering the site.
